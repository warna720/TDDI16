...
main(int argc, char* argv[])
{
  ...
  for(int i{0}; i < N-1; ++i)
  {
    ...
    for(int j{i+1}; j < N; ++j)
    {
      ...
      m[slope].push_back(j);
      mm.insert(make_pair(m[slope].size(),slope));
    }

    std::multimap<int,double>::reverse_iterator it;
    for (it=mm.rbegin(); it->first > 2; ++it)
    {
      int last = m[it->second].size()-1;
      render_line(screen, points.at(i), points.at(m[it->second][last]));
    }
  }
